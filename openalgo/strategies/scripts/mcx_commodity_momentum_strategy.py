...
class MCXMomentumStrategy:
    ...
    def execute_trade(self, action, quantity, reason):
        """Execute trade with strategy parameter."""
        if not self.client:
            logger.warning(f"No API Client. Simulation: {action} {quantity} ({reason})")
            return

        try:
            logger.info(f"Executing {action} {quantity}: {reason}")
            order = self.client.placesmartorder(
                strategy="MCX Momentum",  # REQUIRED first argument
                symbol=self.symbol,
                action=action,
                exchange=self.exchange,
                price_type="MARKET",
    ...
    def check_signals(self):
        ...
        if not has_position:
            ...
            if (current['adx'] > self.params['adx_threshold'] and
                ...
                self.execute_trade('BUY', base_qty, "Entry Signal")
                if self.pm:
            ...
            # SELL Signal: ADX > 25, RSI < 45, Price < Prev Close
                ...
                self.execute_trade('SELL', base_qty, "Entry Signal")
                if self.pm:
        ...
        # Exit Logic
            ...
            if pos_qty > 0: # Long
                if current['rsi'] < 45 or current['adx'] < 20:
                     ...
                     self.execute_trade('SELL', abs(pos_qty), "Exit Signal")
                ...
                if current['rsi'] > 55 or current['adx'] < 20:
                     ...
                     self.execute_trade('BUY', abs(pos_qty), "Exit Signal")
