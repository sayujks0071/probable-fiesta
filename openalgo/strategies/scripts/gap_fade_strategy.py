...
class GapFadeStrategy:
    def __init__(self, api_client, symbol="NIFTY", qty=50, gap_threshold=0.5, logger=None):
        self.client = api_client
        self.symbol = symbol
        self.qty = qty
        self.gap_threshold = gap_threshold # Percentage
        self.pm = PositionManager(f"{symbol}_GapFade")
        self.logger = logger or logging.getLogger("GapFadeStrategy")
    ...
    def execute(self):
        ...
        if gap_pct > self.gap_threshold:
            # Gap UP -> Fade (Short Market) -> Buy Put Option
            self.logger.info("Gap UP detected. Looking to FADE (Short Market -> Buy PE).")
            action = "BUY"
            option_type = "PE"

        elif gap_pct < -self.gap_threshold:
            # Gap DOWN -> Fade (Long Market) -> Buy Call Option
            self.logger.info("Gap DOWN detected. Looking to FADE (Long Market -> Buy CE).")
            action = "BUY"
            option_type = "CE"

        # 3. Select Option Strike (ATM)
        atm = round(current_price / 50) * 50
        # Symbol format varies, just logging intent
...
def main():
    ...
    if not log_file:
        # Default to standard log dir
        log_dir = project_root / "openalgo" / "log" / "strategies"
        log_dir.mkdir(parents=True, exist_ok=True)
        log_file = log_dir / f"gap_fade_{args.symbol}.log"
    ...
    strategy = GapFadeStrategy(client, args.symbol, args.qty, args.threshold, logger=logger)
    strategy.execute()
