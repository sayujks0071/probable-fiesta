...
def run_script(script_path, description):
    ...
    try:
        env = os.environ.copy()
        # Add root and vendor to PYTHONPATH
        # root needed for openalgo_observability (if running from root)
        # vendor needed for 'import openalgo'
        env['PYTHONPATH'] = os.getcwd() + ":" + os.path.join(os.getcwd(), 'vendor') + ":" + env.get('PYTHONPATH', '')

        # Use venv if exists, else system python
        venv_python = os.path.join("vendor", "openalgo", "venv", "bin", "python3")
...
def main():
    ...
    # 2. Daily Prep
    prep_script = os.path.join("vendor", "openalgo", "scripts", "daily_prep.py")
    run_script(prep_script, "Daily Prep")

    # 3. Backtest (Optional)
    if args.backtest:
        backtest_script = os.path.join("vendor", "openalgo", "scripts", "daily_backtest_leaderboard.py")
